...
```{r}
Yhat <- linmod$fitted.values - b0
e <- resid(linmod)
plot(Yhat,e, xlab = expression('Y_i - b_0'),ylab="residual e", pch = 16)
abline(h = 0, lty = 3)
```
...
e <- resid(linmod2)
par(mfrow=c(1,2))
plot(covid_new$percentage_fully_vaccinated,Y1, xlab = expression('percentage_fully_vaccinated'),ylab='Y_i - b_0 - b1 * percentage_vaccinated', pch = 16)
abline(h = 0, lty = 3)
Y2 <- covid_new$perecentage_new_deaths - b0 - b2*covid_new$percentage_fully_vaccinated
plot(covid_new$percentage_vaccinated, Y2, xlab = expression('percentage_vaccinated'),ylab='Y_i - b_0 - b2 * percentage_fully_vaccinated', pch = 16)
abline(h = 0, lty = 3)
```
...
e <- resid(linmod2)
par(mfrow=c(1,2))
plot(agg_new_deaths,Y1, xlab = expression('agg_fully_vaccinated_population'),ylab='Y_i - b_0 - b1 * agg_vaccinated_populations', pch = 16)
abline(h = 0, lty = 3)
Y2 <- agg_new_deaths - b0 - b2*agg_fully_vaccinated_population
plot(agg_new_deaths, Y2, xlab = expression('agg_vaccinated_populations'),ylab='Y_i - b_0 - b2 * agg_fully_vaccinated_population', pch = 16)
abline(h = 0, lty = 3)
```
