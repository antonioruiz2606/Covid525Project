...
For this regression model, the % of People fully Vaccinated and % of People Vaccinated is independent to % of new deaths. This is because the % of vaccinated is not grouped/ aggregated by country thus there are multiple points for a single country at different day's which are making the dependence to each other very difficult. Thus the relation between the residual error is quite independent.
Therefore we are not violating the independence assumption of regression models.
...
The QQ-Plot allows us to visualize the normality of the errors from the data. There is a no tail violation of normality in the QQ-Plot since the data is relatively evenly spread out across the line. The tails don't strafe much from the line. And the distribution is not "compacted" on the ends. The quantiles from residuals distribution almost match quantiles from a normal distribution.
